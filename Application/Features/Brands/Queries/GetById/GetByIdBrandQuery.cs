...
{
	//Id bilgisi gelecek ve IBrandRepository de ilgili metot ile veritabanında sorgu yapacak
	...
	public class GetByIdBrandQueryHandler : IRequestHandler<GetByIdBrandQuery, GetByIdBrandResponse>
	{
		...
		{
			//önce Brand nesnesini veritabanından alalım
			//gelen id ile ilgili data olmayabilir o yüzden nullable olsun
			Brand? brand = await _brandRepository.GetAsync(
				predicate:b=>b.Id==request.Id,
				cancellationToken:cancellationToken
				);

			//sonra aldığım Brand nesnesini GetByIdBrandResponse 'a cevirelim
